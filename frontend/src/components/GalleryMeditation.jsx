...
const GalleryMeditation = () => {
    ...
    const handleSearch = (searchTerm) => {
        axios.get(import.meta.env.VITE_BE_URL + `/api/meditationimages?level=${level}&category=${category}&favMeditations=${favMeditations}&description=${description}`, { withCredentials: true })
            .then((res) => {
                ...
                const filteredVideos = res.data.filter((image) => {
                    const videoCategory = image.category.toLowerCase();
                    const videoLevel = image.level.toLowerCase();
                    const videoTitle = image.title.toLowerCase();
                    const videoDescription = image.description.toLowerCase();
                    const search = searchTerm.toLowerCase();
                    return videoCategory.includes(search) || videoLevel.includes(search) || videoDescription.includes(search) || videoTitle.includes(search)
                });
                setImages(filteredVideos);
            })
    ...
    const handleFavImages = (input) => {
        if (input === favMeditations) {
            setFavMeditations(undefined)
        }
        else {
            setFavMeditations(input)
        }
    }
        ...
        <section>
            <article className="category_container">
                <div className="button_container">
                    <button className="category_button" onClick={() => { setFavMeditations(undefined); setLevel(undefined); setCategory(undefined); handleButtonCategory("all") }}><img src={allButtonClicked ? allIconActive : allIcon} alt="" /></button>
